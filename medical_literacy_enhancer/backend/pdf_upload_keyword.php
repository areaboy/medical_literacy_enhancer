...
$('#loader_keyword2').fadeIn(400).html('<br><div style=color:black;background:#ddd;padding:10px;><img src=loader.gif style=font-size:20px> &nbsp;Please Wait! .Extracting Text from the Uploaded Medical Files and Performing Keyword Analysis</div>');
...
$.ajax({
			...
			type:'POST',
			url:'backend/pdf_extraction_openai_geminiai_keyword.php',
			data:datasend,
                        crossDomain: true,
			cache:false,
			success:function(msg){

                        $('#loader_keyword2').hide();
				//$('#result_ex').fadeIn('slow').prepend(msg);
$('#result_keyword2').html(marked.parse(msg));
$('#alerts_keyword2').delay(5000).fadeOut('slow');
$('#alerts_keyword2a').delay(5000).fadeOut('slow');
$('#file_content8').val('');			
			}
...
<br>
<div class=''>
<div id='loader_keyword2'></div>
<div id='result_keyword2'></div>
</div>
