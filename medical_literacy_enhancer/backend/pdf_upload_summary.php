...
if ($sentence == ''){
echo "<div style='background:red;color:white;padding:8px;border:none;'>No. of Sentences for Medical File Summarization Cannot be Empty</div>";
exit();
}
...
$('#loader_summarize2').fadeIn(400).html('<br><div style=color:black;background:#ddd;padding:10px;><img src=loader.gif style=font-size:20px> &nbsp;Please Wait! .Extracting Text from the Uploaded Medical Files and Performing Summarization Analysis</div>');
var datasend = {file_name:file_name,chatbot_ai:chatbot_ai, sentence:sentence};


$.ajax({
			
			type:'POST',
			url:'backend/pdf_extraction_openai_geminiai_summary.php',
			data:datasend,
                        crossDomain: true,
			cache:false,
			success:function(msg){

                        $('#loader_summarize2').hide();
				//$('#result_ex').fadeIn('slow').prepend(msg);
$('#result_summarize2').html(marked.parse(msg));
$('#alerts_summarize2').delay(5000).fadeOut('slow');
$('#alerts_summarize2a').delay(5000).fadeOut('slow');
$('#file_content2').val('');
$('#sentence2').val('');			
			}
...
<br>
<div class=''>
<div id='loader_summarize2'></div>
<div id='result_summarize2'></div>
</div>
