...
$('#loader_sentiment2').fadeIn(400).html('<br><div style=color:black;background:#ddd;padding:10px;><img src=loader.gif style=font-size:20px> &nbsp;Please Wait! .Extracting Text from the Uploaded Medical Files and Performin Sentiments Analysis</div>');
var datasend = {file_name:file_name,chatbot_ai:chatbot_ai};


$.ajax({
			
			type:'POST',
			url:'backend/pdf_extraction_openai_geminiai_sentiment.php',
			data:datasend,
                        crossDomain: true,
			cache:false,
			success:function(msg){

                        $('#loader_sentiment2').hide();
				//$('#result_ex').fadeIn('slow').prepend(msg);
$('#result_sentiment2').html(marked.parse(msg));
$('#alerts_sentiment2').delay(5000).fadeOut('slow');
$('#alerts_sentiment2a').delay(5000).fadeOut('slow');
$('#file_content').val('');			
			}
...
<br>
<div class=''>
<div id='loader_sentiment2'></div>
<div id='result_sentiment2'></div>
</div>
