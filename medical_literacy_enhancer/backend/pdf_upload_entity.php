...
$('#loader_entity2').fadeIn(400).html('<br><div style=color:black;background:#ddd;padding:10px;><img src=loader.gif style=font-size:20px> &nbsp;Please Wait! .Extracting Text from the Uploaded Medical Files and Performing Entities Analysis</div>');
...
$.ajax({
			...
			type:'POST',
			url:'backend/pdf_extraction_openai_geminiai_entity.php',
			data:datasend,
                        crossDomain: true,
			cache:false,
...
$('#result_entity2').html(marked.parse(msg));
$('#alerts_entity2').delay(5000).fadeOut('slow');
$('#alerts_entity2a').delay(5000).fadeOut('slow');
$('#file_content3').val('');			
			}
...
<br>
<div class=''>
<div id='loader_entity2'></div>
<div id='result_entity2'></div>
</div>
